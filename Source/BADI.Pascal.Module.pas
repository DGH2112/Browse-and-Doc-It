...
Type
  ...
  **)
  ...
  Strict Protected
    ...
    Function  InLineVarDecl : Boolean;
    Function  InLineConstDecl : Boolean;
    ...
    Function  IsForVarDecl : Boolean;
    Function  IsForIndentifier : Boolean;
...
Begin
  ...
  Try
    ...
    If Assigned(M) Then
      ...
      End;
  Finally
    If Not Assigned(Container) And Assigned(M) Then
      M.Free;
  End;
End;
...
Begin
  ...
  If Result Then
    Begin
      NextNonCommentToken;
      If IsForVarDecl Or IsForIndentifier Then
        Begin
          If Token.Token = ':=' Then
            Begin
              NextNonCommentToken;
...
Begin
  ...
  If Result Then
    Begin
      If Assigned(CurrentMethod) Then
        If mmsoMethodCCIncIF In BADIOptions.ModuleMetricSubOptions Then
          Begin
      ...
      Try
        ...
        If mmsoMethodCCIncIF In BADIOptions.ModuleMetricSubOptions Then
          If mmsoMethodCCIncludeExpression In BADIOptions.ModuleMetricSubOptions Then
...
(**
  ...
  This method processes an in-line constant declaration within a block.

  @precon  None.
  @postcon The in-line constant is parsed and the method returns true if found else returns false.

  @return  a Boolean

**)
Function TPascalModule.InLineConstDecl: Boolean;
            ...
            Begin
              ...
              Try
                ...
                If Assigned(Cmt) Then
                  Begin
                    C.Comment := TPascalComment.Create(Cmt);
                    OwnedItems.Add(C.Comment);
                  End;
              Finally
                Tmp.Free;
              End;
...
Begin
  If Not (InLineVarDecl Or InLineConstDecl) Then
    Statement;
End;

(**

  This method parses an in-line variable declaration in the code block.

  @precon  None.
  @postcon The variables are parsed and added the list of method variables and returns true if found else
           returns false.

  @return  a Boolean

**)
Function TPascalModule.InLineVarDecl: Boolean;
  ...
  (**
    ...
    @param   T     as a TTypes as a constant

  **)
  Procedure CreateVariable(Const Ident : TElementContainer; Const T : TTypes);
  ...
  Begin
    ...
    If Assigned(T) Then
      V.AddTokens(T);
    If Assigned(Ident.Comment) Then
      Begin
        V.Comment := TPascalComment.Create(Ident.Comment);
        OwnedItems.Add(V.Comment);
      End;
  End;

Var
  I, Tmp : TElementContainer;
  T: TTypes;
  AToken: TTokenInfo;
  j: Integer;
  ExprType: TPascalExprTypes;
...
Begin
  ...
  If Result Then
    Begin
      NextNonCommentToken;
      I := TTempCntr.Create('', scLocal, 0, 0, iiNone, Nil);
      Try
        IdentList(I, scLocal, TVar, [':', ':=', ';']);
        If Token.Token = ':' Then
          Begin
            NextNonCommentToken;
            AToken := Token;
          ...
          Else
            ...
            Try
              ...
              For j := 1 To I.ElementCount Do
                CreateVariable(I[j], T);
            Finally
              Tmp.Free;
            End;
          End Else
            For j := 1 To I.ElementCount Do
              CreateVariable(I[j], Nil);
        If Token.Token = ':=' Then
          Begin
            NextNonCommentToken;
            ExprType := [petUnknown];
...
(**
  ...
  This method checks the current token for being an Identifier.

  @precon  None.
  @postcon If the current token is an identifier it is processed and the function returns true else
           returns false if not an identifier.

  @return  a Boolean

**)
Function TPascalModule.IsForIndentifier: Boolean;
...
**)
Function TPascalModule.IsForVarDecl: Boolean;
        ...
        Begin
          ...
          If Token.Token = ':' Then
            Begin
              ...
              Tmp := TTempCntr.Create('', scLocal, 0, 0, iiNone, Nil);
              Try
                //T := GetTypeDecl(TypeToken(Nil, scLocal, Nil, Tmp));
                While IsIdentifier(Token) Do
                  Begin
                    V.AddToken(Token.Token, Token.TokenType);
                    NextNonCommentToken;
                    If Token.Token = '.' Then
                      Begin
                        V.AddToken(Token.Token, Token.TokenType);
                ...
                If Assigned(Cmt) Then
                  Begin
                    V.Comment := TPascalComment.Create(Cmt);
                    OwnedItems.Add(V.Comment);
                  End;
              Finally
                Tmp.Free;
      ...
      If Result Then
        Begin
          ...
          Repeat
            If IsIdentifier(Token) Then
              Begin
                CurrentMethod.LabelsLabel[True].Add(Token, scLocal, iiPublicLabel, GetComment);
                NextNonCommentToken;
              End Else
  ...
  If Result Then
    Begin
      ...
      If Token.UToken = strUNTIL Then
        Begin
          If Assigned(CurrentMethod) Then
            If mmsoMethodCCIncREPEAT In BADIOptions.ModuleMetricSubOptions Then
              CurrentMethod.IncCyclometricComplexity;
          NextNonCommentToken;
          ExprType := [petUnknown];
...
Begin
  If CurrentMethod <> Nil Then
    Begin
      If Assigned(CurrentMethod.LabelsLabel[False]) Then // Check for label
        If CurrentMethod.LabelsLabel[True].FindElement(Token.Token) <> Nil Then
          Begin
            NextNonCommentToken;
            If Token.Token = ':' Then
              NextNonCommentToken
...
Begin
  ...
  If Result Then
    Begin
      If Assigned(CurrentMethod) Then
        If mmsoMethodCCIncWHILE In BADIOptions.ModuleMetricSubOptions Then
          CurrentMethod.IncCyclometricComplexity;
      NextNonCommentToken;
      ExprType := [petUnknown];
