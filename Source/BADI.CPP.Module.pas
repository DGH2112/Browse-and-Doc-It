(**
  ...
  C++ Module : A unit to tokenize C++ source code.

  @note       This unit ONLY tokenizes C++ at the moment - the parser HAS NOT BEEN WRITTEN!

  @nometrics @nohints @stopdocumentation @nochecks
...
//  TCPPMethod = Class(TGenericMethodDecl)
//  {$IFDEF D2005} Strict {$ENDIF} Private
...
//  TRefTypes = Set of TRefType;
  ...
  (**
    ...
    This is the main class for dealing with C++ units and program source files. It creates
    and manages instances of the other classes as needed.

  **)
  TCPPModule = Class(TBaseLanguageModule)
  {$IFDEF D2005} Strict {$ENDIF} Private
    FSource                  : String;
...
//    Procedure Block(AScope : TScope; Method : TCPPMethod);
...
//    Function ProcedureDecl(AScope : TScope) : TCPPMethod;
//    Function FunctionDecl(AScope : TScope) : TCPPMethod;
//    Function ConstructorDecl(AScope : TScope) : TCPPMethod;
//    Function DestructorDecl(AScope : TScope) : TCPPMethod;
//    Function OperatorDecl(AScope : TScope) : TCPPMethod;
//    Function FunctionHeading(AScope :TScope; Container : TElementContainer; boolIdent : Boolean = True) : TCPPMethod;
//    Function ProcedureHeading(AScope : TScope; Container : TElementContainer; boolIdent : Boolean = True) : TCPPMethod;
//    Function OperatorHeading(AScope :TScope; Container : TElementContainer; boolIdent : Boolean = True) : TCPPMethod;
//    Procedure FormalParameter(Method : TCPPMethod);
//    Procedure FormalParam(Method : TCPPMethod);
//    Procedure Parameter(Method : TCPPMethod; ParamMod : TParamModifier);
//    Procedure Directive(M : TCPPMethod; boolGrammarFix : Boolean = False);
...
//    Function ConstructorHeading(AScope :TScope; Container : TElementContainer) : TCPPMethod;
//    Function DestructorHeading(AScope :TScope; Container : TElementContainer) : TCPPMethod;
...
//    Function CheckReturnValue(Method : TCPPMethod) : Boolean;
//    Procedure CheckAlias(Method : TCPPMethod);
...
//    Procedure AddToContainer(Container : TElementContainer; var Method : TCPPMethod);
...
//    Procedure EatDotNETAttribute;
  {$IFDEF D2005} Strict {$ENDIF} Protected
//    function GetCurrentMethod: TCPPMethod;
//    Function GetModuleName : String; Override;
//    Function GetComment(
//      CommentPosition : TCommentPosition = cpBeforeCurrentToken) : TComment;
//      Override;
//    Procedure CheckFunctionReturn(Func : TCPPMethod);
//    (**
...
//      @return  a TCPPMethod
//    **)
//    Property CurrentMethod : TCPPMethod Read GetCurrentMethod;
//    (**
...
Const
  ...
  **)
  strReservedWords : Array[0..98] Of String = (
    '__alignment_of',
    '__classid',
    '__declspec',
    '__except',
  ...
  directives. **)
  strDirectives : Array[0..57] Of String = (
    '__automated',
    '__cdecl',
    '__classid',
    '__classmethod',
...
**)
Class function TCPPComment.CreateComment(Const strComment: String; Const iLine, iCol: Integer): TComment;
...
begin
  ...
  If Length(strText) > 0 Then
    Begin
      Case strText[1] Of
        //: @debug '/' : strText := Copy(strText, 3, Length(strText) - 2);
...
//
//**)
//Procedure TCPPMethod.AddDirectives(strDirective : String);
//
//Begin
...
//(**
//
//  This is the constructor method for the TCPPMethod class.
//
//  @precon  None.
...
//
//**)
//constructor TCPPMethod.Create(MethodType: TMethodType; const strName : String;
...
//(**
//
//  This is the destructor method for the TCPPMethod class.
//
//  @precon  None.
//  @postcon Frees the memory used for the directives string list.
//
//**)
//destructor TCPPMethod.Destroy;
//begin
//  FDirectives.Free;
//  Inherited Destroy;
//end;
...
//
//**)
//function TCPPMethod.AsString(boolShowIdentifier,
//  boolForDocumentation : Boolean): String;
//
//Var
//  i : Integer;
//
//begin
//  Result := 'FIX'; //: @debug strMethodTypes[MethodType];
//  If Name <> '' Then
//    If boolShowIdentifier Then
...
//
//**)
//function TCPPMethod.GetName: String;
//
//Var
//  i : Integer;
//
...
//  For i := 0 To ParameterCount - 1 Do
//    Begin
//      //: @debug Result := Result + '.' + strParamModifier[Parameters[i].ParamModifier];
...
//
//**)
//function TCPPMethod.HasDirective(strDirective: String): Boolean;
//
//Var
//  i : Integer;
//
...
//
//**)
//Function TCPPMethod.ReferenceSymbol(AToken : TTokenInfo) : Boolean;
//
//Var
//  i: Integer;
//  M: TCPPMethod;
...
//    If Elements[i] Is TCPPMethod Then
//      Begin
//        M := Elements[i] As TCPPMethod;
...
//
//**)
//constructor TObjectDecl.Create(const strName: String; AScope: TScope; iLine,
//  iColumn: Integer; iImageIndex: TBADIImageIndex; AComment: TComment);
//begin
//  Inherited Create(strName, AScope, iLine, iColumn, iImageIndex, AComment);
...
**)
Constructor TCPPModule.CreateParser(const Source, strFileName : String; Const IsModified : Boolean;
...
**)
Procedure TCPPModule.TokenizeStream;
    ...
    Begin
              ...
              If iTokenLen > 0 Then
                If Not (IsInSet(strToken[1], strWhiteSpace + strLineEnd)) Then
                  Begin
                    ...
                    If BlockType = btCompilerDirective Then
                      LastCharType := ttCompilerDirective;
      ...
      If Ch = #10 Then
        Begin
          Inc(iLine);
          iColumn := 1;
          If BlockType In [btLineComment, btStringLiteral, btCompilerDirective] Then
            BlockType := btNoBlock;
        End;
      LastChar := Ch;
    End;
...
//        End;
//      tmpMethod := Method;
//      Method := Container.Add(tmpMethod) As TCPPMethod;
...
**)
function TCPPModule.ReservedWords: TKeyWords;
...
**)
function TCPPModule.Directives: TKeyWords;
...
//
//**)
//function TCPPModule.GetCurrentMethod: TCPPMethod;
//begin
//  If FMethodStack.Count = 0 Then
//    Result := Nil
//  Else
//    Result := FMethodStack[FMethodStack.Count - 1] As TCPPMethod;
//end;
  ...
  Try
    If TokenCount > 0 Then
      Begin
        ...
        While (Token.TokenType In [ttLineComment, ttBlockComment, ttCompilerDirective]) And
          Not EndOfTokens Do
          NextNonCommentToken;
...
//
//  @param   AScope as a TScope
//  @param   Method as a TCPPMethod
//
//**)
//procedure TPascalModule.Block(AScope : TScope; Method : TCPPMethod);
//
//Var
//  bool : Boolean;
//
...
**)
Class Function TCPPModule.DefaultProfilingTemplate: String;
...
//        LabelScope := scPublic;
//      If Container Is TCPPMethod then
//        Begin
//          If (Container As TCPPMethod).ConstantsLabel = Nil Then
//            (Container As TCPPMethod).ConstantsLabel := CurrentMethod.Add(
//              strConstantsLabel, iiPublicConstantsLabel, LabelScope,
//              GetComment) As TLabelContainer;
//          C := (Container As TCPPMethod).ConstantsLabel;
//        End Else
...
//      If Container Is TCPPMethod Then
//        Begin
//          If (Container As TCPPMethod).TypesLabel = Nil Then
//            (Container As TCPPMethod).TypesLabel := Container.Add(strTypesLabel,
//              iiPublicTypesLabel, LabelScope, GetComment) As TLabelContainer;
//          TL := (Container As TCPPMethod).TypesLabel;
//        End Else
//      If Container Is TClassDecl Then
//        Begin
...
**)
Function TCPPModule.ReferenceSymbol(Const AToken : TTokenInfo) : Boolean;
...
//      If Container Is TCPPMethod Then
//        Begin
//          If (Container As TCPPMethod).VariablesLabel = Nil Then
//            (Container As TCPPMethod).VariablesLabel := Container.Add(
//              strVarsLabel, iiPublicVariablesLabel, LabelScope,
//              GetComment) As TLabelContainer;
//          V := (Container As TCPPMethod).VariablesLabel;
//        End Else
//      If Container Is TClassDecl Then
//        Begin
...
**)
procedure TCPPModule.TidyUpEmptyElements;
...
**)
function TCPPModule.AsString(Const boolShowIdentifier, boolForDocumentation : Boolean): String;

begin
  Result := ''; //: @debug strModuleTypes[ModuleType];
  If boolShowIdentifier Then
...
//
//**)
//Function TPascalModule.ProcedureDecl(AScope : TScope) : TCPPMethod;
//
//Begin
...
//
//**)
//Function TPascalModule.FunctionDecl(AScope : TScope) : TCPPMethod;
//
//Begin
...
//
//**)
//Function TPascalModule.ConstructorDecl(AScope : TScope) : TCPPMethod;
//
//Begin
...
//
//**)
//Function TPascalModule.DestructorDecl(AScope : TScope) : TCPPMethod;
//
//Begin
...
//
//**)
//Function TPascalModule.OperatorDecl(AScope : TScope) : TCPPMethod;
//
//Begin
...
//  Container : TElementContainer; boolIdent : Boolean = true) : TCPPMethod;
//
//Var
//  C : TComment;
...
//            End;
//        Finally
//          Result := TCPPMethod.Create(mtFunction, strIdentifier, AScope,
//            iLine, iColumn);
...
//            End;
//        Finally
//          Result := TCPPMethod.Create(mtOperator, strIdentifier, AScope,
//            iLine, iColumn);
//          Result.ClassNames.Assign(slClassNames);
//          Result.ObjClsInt := FindObjClsInt(slClassNames);
//          Result.Comment := C;
...
//
//  @precon  Method must be a valid TCPPMethod instance.
//  @postcon Checks the alias (if one exists) of the procedure / function.
//
//  @param   Method as a TCPPMethod
//
//**)
//procedure TPascalModule.CheckAlias(Method : TCPPMethod);
//
//Begin
//  If Token.Token = '=' Then
//    Begin
...
//           external functions. If not present adds an issue.
//
//  @param   Func as a TCPPMethod
//
//**)
//procedure TPascalModule.CheckFunctionReturn(Func : TCPPMethod);
//
//begin
...
//
//**)
//Function TPascalModule.CheckReturnValue(Method : TCPPMethod) : Boolean;
//
//Begin
//  Result := False;
//  If Token.Token = ':' Then
...
**)
procedure TCPPModule.CheckUnResolvedMethods;
...
begin
  ...
  //: @debug ResolveScopeOfImplementedExportedMethods;
  //: @debug ResolveScopeOfImplementedExportsMethods;
  ...
  If Errors <> Nil Then
    If Errors.ElementCount > 0 Then
      Exit;
  //: @debug FindUnresolvedObjectAndClassMethods(FTypesLabel);
  //: @debug FindUnresolvedExportedMethods;
  {FindUnresolvedExportsMethods;}
  //: @debug FindUnresolvedImplementedClassMethods(FImplementedMethodsLabel);
end;
...
//              End;
//        Finally
//          Result := TCPPMethod.Create(mtProcedure, strIdentifier, AScope,
//            iLine, iColumn);
//          Result.ClassNames.Assign(slClassNames);
//          Result.ObjClsInt := FindObjClsInt(slClassNames);
//          Result.Comment := C;
...
//
//**)
//Procedure TPascalModule.FormalParameter(Method : TCPPMethod);
//
//Begin
//  If Token.Token = '(' Then
//    Begin
...
//
//**)
//Procedure TPascalModule.FormalParam(Method : TCPPMethod);
//
//Var
...
//
//**)
//Procedure TPascalModule.Parameter(Method : TCPPMethod;
...
//
//  @param   M              as a TCPPMethod
//  @param   boolGrammarFix as a Boolean
//
//**)
//Procedure TPascalModule.Directive(M : TCPPMethod; boolGrammarFix : Boolean = False);
//
//Var
//  C : TElementContainer;
//  ExprType : TExprTypes;
...
//  Container : TElementContainer): TCPPMethod;
//
//Var
//  C : TComment;
//  strIdentifier: String;
...
//                iColumn);
//            Finally
//              Result := TCPPMethod.Create(mtConstructor, strIdentifier, AScope,
//                iLine, iColumn);
//              Result.ClassNames.Assign(slClassNames);
//              Result.ObjClsInt := FindObjClsInt(slClassNames);
//              Result.Comment := C;
...
//                iColumn);
//            Finally
//              Result := TCPPMethod.Create(mtDestructor, strIdentifier, AScope,
//                iLine, iColumn);
//              Result.ClassNames.Assign(slClassNames);
//              Result.ObjClsInt := FindObjClsInt(slClassNames);
//              Result.Comment := C;
...
procedure TCPPModule.ProcessCompilerDirective(var iSkip : Integer);
  ...
  **)
  Procedure IncSkip(Const iValue : Integer);

  Begin
//    CompilerConditionStack.Push(ccIncludeCode, TokenIndex);
//    Inc(iSkip, iValue);
  End;
...
//      Begin
//        If StartLabel.Elements[k] Is TCPPMethod Then
//          Begin
//            Method := StartLabel.Elements[k] As TCPPMethod;
...
//      If FExportedHeadingsLabel.Elements[k] Is TCPPMethod Then
//        Begin
//          Method := FExportedHeadingsLabel.Elements[k] As TCPPMethod;
...
//
//  **)
//  Function ShouldMethodHaveImplementation(Method : TCPPMethod) : Boolean;
//
//  Begin
...
//                If MethodsLabel.Elements[j] Is TCPPMethod Then
//                  Begin
//                    Method := MethodsLabel.Elements[j] As TCPPMethod;
...
//          If (ImplementedMethod <> Nil) And (ImplementedMethod Is TCPPMethod) Then
//            Begin
//              Method.Resolved := True;
//              (ImplementedMethod As TCPPMethod).Resolved := True;
//              (ImplementedMethod As TCPPMethod).Scope := Method.Scope;
//            End;
//        End;
//end;
//
...
//
//**)
//procedure TCPPModule.ResolveScopeOfImplementedClassMethods(
//  StartLabel : TLabelContainer);
//
//var
...
//      If StartLabel.Elements[i] Is TCPPMethod Then
//        Begin
//          Method := StartLabel.Elements[i] As TCPPMethod;
